...
class Table:
    ...
    def construct_full_record(self, rid, relative_version=0):
        ...
        visible_versions = [base_values[:]]
        current = base_values[:]

        for tail_entry in reversed(tail_chain):

            tail_values = [_read(tail_entry, i + 3) for i in range(self.num_columns)]

            is_copy_tail = True

            for i in range(self.num_columns):
                if tail_values[i] != base_values[i]:
                    is_copy_tail = False
                    break
            ...
            for i in range(self.num_columns):
                if tail_values[i] is not None and tail_values[i] != current[i]:
                    current[i] = tail_values[i]
                    changed = True

            if changed:
                visible_versions.append(current[:])

        latest_index = len(visible_versions) - 1
        target_index = latest_index + relative_version

        if target_index < 0:
            target_index = 0

        return visible_versions[target_index]
