...
class Table:
    ...
    def __init__(self, name, num_columns, key):
        ...
        self.lock_manager = None  # Set by Database
        self.directory_lock = threading.Lock() # Protects page_directory and page_range_directory

    def __getstate__(self):
        """Exclude unpicklable threading.Lock from serialization."""
        state = self.__dict__.copy()
        for lock_name in ['_merge_lock', 'directory_lock']:
            if lock_name in state:
                del state[lock_name]
        # lock_manager is set by the database on open
        if 'lock_manager' in state:
            del state['lock_manager']
        return state
    ...
    def get_column_value(self, rid, column_number, relative_version=0):
        ...
        if base_schema_int == 0:
            # No updates exist, return base value directly
        ...
        version_num = 0
        while (indirection_rid != base_rid and indirection_rid != None and indirection_rid != 0):
            if relative_version == 0 and base_page_range.tps > 0 and indirection_rid <= base_page_range.tps:
                break
