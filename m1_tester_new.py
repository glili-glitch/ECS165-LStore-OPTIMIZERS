...
for i in range(0, number_of_records):
    ...
    if not query.insert(*records[key]):
        print("error: insert failed on", records[key])
    else:
        pass
    # print('inserted', records[key])
print("Insert finished")

# Test aborting when inserting duplicate key
for key in records:
    if query.insert(*[key, randint(0, 20), randint(0, 20), randint(0, 20), randint(0, 20)]):
        print("error: duplicate key inserted")
    else:
        pass
print("Duplicate key insert test finished")

# Check inserted records using select query
# for key in records:
#     print('select on', key, ':', records[key])

test_value = randint(0, 20)
test_column = randint(1, 4)
test_select_records = query.select(test_value, test_column, [1, 1, 1, 1, 1])
print(f"Number of records with {test_value} in column {test_column}:", len(test_select_records))
# for record in test_select_records:
...
print("Select version -1 test finished")

# Test aborting when updating primary key to a conflicting key
for key in records:
    updated_columns = [choice(list(records)), None, None, None, None]
    if query.update(key, *updated_columns):
        print("error: illegal update")
    else:
        pass

print("Illegal update test finished")
